...
def make_ensemble(
    ...
    # def binary_log_loss_langevin_gradient(y_pred, y_true):
    #     p = sigmoid(y_pred) #second object is of class lightgbm.basic.Dataset
    #     # p = _positive_sigmoid(y_pred) #y_pred is object of class lightgbm.basic.Dataset
    #     grad = p - y_true
    #     hess = p * (1 - p)
    #     return grad, hess
    #
    # # z = uniform(10, 0.5, 3.5)
    # z = np.empty(shape = 20, dtype = "float")
    # for i in range(20):
    #     z[i] = random.uniform(0.5, 3.5)
    ...
    # z = uniform(10, 0.5, 3.5)
    # z = np.empty(shape = len(lgb_data.get_label()), dtype = "float")
    # for i in range(len(z)):
    #     z[i] = random.uniform(0.5, 3.5)


    def lgb_objective(z, lgb_data):
        t = lgb_data.get_label().astype(int)
        t[t == 0] = -1
        ...
        r = np.empty(shape = len(t), dtype = "float")
        for i in range(len(t)):
            r[i] = lgb_sigmoid(t[i], z[i])

        grad = r
        hess = np.abs(r) * (1 - np.abs(r))

        return grad, hess



    y_pred = np.array([0.8, 0.9, 0.2])
    y_true = np.array([1, 0, 0])
    # def binary_objective(y_true, y_pred):
    #     # y = sigmoid(z)
    #     grad = y_pred - y_true
    ...
    # one_sglb_modeler.config["fobj"] = lgb_objective
    ...
    forecasts
    pd.Series(sigmoid(forecasts2['1-period Survival Probability']))
    pd.Series(sigmoid(forecasts2['6-period Survival Probability']))

    np.var(forecasts)
    np.var(sigmoid(forecasts2['6-period Survival Probability']))
